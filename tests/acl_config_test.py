...
from config.main import config as config

from click.testing import CliRunner
# from config.main import expand_vlan_ports, parse_acl_table_info
from config.acl import parse_acl_table_info
...
class TestConfigAcl(object):
    ...
    def test_parse_table_with_invalid_ports(self):
        with pytest.raises(Exception):
            parse_acl_table_info(ContextMock(), "TEST", "L3", None, "Ethernet200", "egress")

    def test_parse_table_with_empty_ports(self):
        with pytest.raises(Exception):
            parse_acl_table_info(ContextMock(), "TEST", "L3", None, "", "egress")

    def test_acl_add_table_nonexistent_port(self):
        runner = CliRunner()

        result = runner.invoke(
            config, ["acl", "add", "table", "TEST", "L3", "-p", "Ethernet200"])
    ...
    def test_acl_add_table_empty_string_port_list(self):
        ...
        result = runner.invoke(
            config, ["acl", "add", "table", "TEST", "L3", "-p", ""])
